...
async function cargar() {
  try {
    const res = await fetch('/.netlify/functions/celulares');
    const data = await res.json();
    productos = Array.isArray(data.productos) ? data.productos : [];
    if (!productos.length) throw new Error('Sin productos');
    document.getElementById('estado').textContent = `${productos.length} celulares encontrados`;
    document.getElementById('ultimaActualizacion').textContent = data.fuente || 'Catálogo cargado';
    poblarMarcas(productos);
    render();
    actualizarCarrito();
  } catch (error) {
    document.getElementById('estado').textContent = 'No se pudo cargar el catálogo';
    document.getElementById('ultimaActualizacion').textContent = 'Revisá la función de Netlify';
    console.error(error);
  }
}
...
async function verDetalle(id) {
  ...
  if (!detalle && p.url) {
    try {
      const res = await fetch(`/.netlify/functions/detalle?url=${encodeURIComponent(p.url)}`);
  ...
  panel.innerHTML = `
    ...
    <div class="specs">
      ...
      <p><b>SKU:</b> ${d.sku || '-'}</p>
      <p><b>Descripción:</b> ${d.descripcion || 'Detalle tomado desde Evophone al abrir el producto.'}</p>
    </div>
    <div class="detail-actions">
      ...
      ${p.url ? `<a class="linkbtn" target="_blank" rel="noopener" href="${p.url}">Ver en Evophone</a>` : ''}
    </div>
  `;
}
