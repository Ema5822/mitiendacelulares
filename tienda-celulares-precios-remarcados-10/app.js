...
async function cargar() {
  ...
  try {
    ...
    productos = (Array.isArray(data.productos) ? data.productos : []).map(p=>({...p, precio: Number(p.precio)*1.10, precio_original: p.precio_original ? Number(p.precio_original)*1.10 : null}));
...
async function verDetalle(id) {
  ...
  const d = { ...p, ...(detalle || {}) };
  const precioDetalle = d.precio ? Number(d.precio) * 1.10 : Number(p.precio || 0);
  const precioOriginalDetalle = d.precio_original ? Number(d.precio_original) * 1.10 : null;
  panel.innerHTML = `
    <button class="close-detail" onclick="cerrarDetalle(event)">✕</button>
    ...
    <div class="price-old">${precioOriginalDetalle ? formatoPrecio(precioOriginalDetalle) : ''}</div>
    <p class="precio precio-detalle">${formatoPrecio(precioDetalle)}</p>
    <div class="specs">
    ...
    <div class="detail-actions">
      ...
      <a class="wa" target="_blank" rel="noopener" href="https://wa.me/5493405504914?text=${encodeURIComponent('Hola, quiero comprar ' + p.nombre + ' - Precio: ' + formatoPrecio(precioDetalle))}">Comprar por WhatsApp</a>
